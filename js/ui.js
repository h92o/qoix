...
const UI = (() => {
  ...
  function syncGranularUI() {
    if (typeof GranularEngine === 'undefined') return;
    const g = GranularEngine.getState();
    setCheck('gr-enabled', g.enabled);

    // Segmented controls
    const seg = (containerId, dataKey, value) => {
      const el = $(containerId);
      if (!el) return;
      el.querySelectorAll('.seg-btn').forEach(b =>
        b.classList.toggle('active', b.dataset[dataKey] === value));
    };
    seg('gr-source-seg',    'source',   g.source);
    seg('gr-osc-wave-seg',  'wave',     g.oscWave);
    seg('gr-window-seg',    'window',   g.grain.window);
    seg('gr-scan-mode-seg', 'scanmode', g.scan.mode);

    // The oscillator wave row only applies to the oscillator source
    const showOsc = g.source === 'oscillator';
    const oscRow = $('gr-osc-wave-row');
    const oscSeg = $('gr-osc-wave-seg');
    if (oscRow) oscRow.style.display = showOsc ? '' : 'none';
    if (oscSeg) oscSeg.style.display = showOsc ? '' : 'none';

    setRange('gr-size',        g.grain.size,          fmtGranular);
    setRange('gr-sizespray',   g.grain.sizeSpray,     fmtGranular);
    setRange('gr-density',     g.grain.density,       fmtGranular);
    setRange('gr-jitter',      g.grain.timingJitter,  fmtGranular);
    setRange('gr-position',    g.grain.position,      fmtGranular);
    setRange('gr-posspray',    g.grain.positionSpray, fmtGranular);
    setRange('gr-pitchspray',  g.grain.pitchSpray,    fmtGranular);
    setRange('gr-panspread',   g.grain.panSpread,     fmtGranular);
    setRange('gr-reverse',     g.grain.reverse,       fmtGranular);
    setRange('gr-scan-period', g.scan.period,         fmtGranular);

    setCheck('gr-scan-enabled', g.scan.enabled);
    setCheck('gr-freeze',       g.scan.freeze);

    setRange('gr-env-a', g.env.attack,  fmtGranular);
    setRange('gr-env-d', g.env.decay,   fmtGranular);
    setRange('gr-env-s', g.env.sustain, fmtGranular);
    setRange('gr-env-r', g.env.release, fmtGranular);

    // A patch cannot carry the sample audio, so this reflects whatever
    // is actually loaded rather than what the patch named.
