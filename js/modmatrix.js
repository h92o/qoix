...
const ModMatrix = (() => {
  ...
  // Pre-dialled amounts, all switched off: the dots are there to be
  // turned on, not to change the sound of a fresh patch. (Velocity is
  // already applied to the amp envelope inside the engine, so routing
  // it to Amplitude here as well would double-scale every note.)
  matrix.lfo1.pitch.amount         = 0.1;
  matrix.lfo2.filter_cut.amount    = 0.4;
  matrix.velocity.amp.amount       = 0.5;
  ...
  // Evaluate one cycle of the selected LFO2 shape at phase p (0..2pi).
  function lfoShape(wave, p) {
    const x = (p / (Math.PI * 2)) % 1;   // 0..1 within the cycle
    switch (wave) {
      case 'square':   return x < 0.5 ? 1 : -1;
      case 'sawtooth': return x * 2 - 1;
      case 'triangle': return x < 0.5 ? (x * 4 - 1) : (3 - x * 4);
      case 'sine':
  ...
  function tick(dt, lfo1Val, envVal, noteVal, velVal) {
    ...
    // LFO2. Its rate is itself a modulation destination, so fold in
    // last frame's lfo2_rate result before advancing the phase.
    const rate = Math.max(0.01, lfo2State.rate + (modValues.lfo2_rate || 0) * 10);
    _lfo2Phase += rate * dt * Math.PI * 2;
    // Wrap, or the phase loses float precision over a long session.
    if (_lfo2Phase > Math.PI * 2) _lfo2Phase %= Math.PI * 2;
    const lfo2Val = lfoShape(lfo2State.wave, _lfo2Phase) * lfo2State.depth;

    // Random (sample-and-hold)
    _randomTimer -= dt;
    if (_randomTimer <= 0) {
      _randomValue = Math.random() * 2 - 1;
      _randomTimer = 1 / Math.max(0.1, rate); // sync to lfo2 rate
    }
  ...
  // Clear every route. Used when loading a patch that carries no matrix
  // so stale routings from the previous patch cannot bleed through.
  function clearAll() {
    SOURCES.forEach(src => DESTINATIONS.forEach(dst => {
      matrix[src.id][dst.id].enabled = false;
    }));
    DESTINATIONS.forEach(d => { modValues[d.id] = 0; });
  }
...
if (typeof module !== 'undefined' && module.exports) module.exports = ModMatrix;
